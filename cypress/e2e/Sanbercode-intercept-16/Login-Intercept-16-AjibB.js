// Tugas Hari 16 Sanbercode - Ajib Bahauddin

describe("Tugas Hari 16 Sanbercode Testing Login Website Orange HRM by Ajib Bahauddin",  () => {
   it("LOG_001-Successfully login with valid username and valid password ", () => {
    ...
    cy.visit('https://opensource-demo.orangehrmlive.com/')
    cy.get(':nth-child(2) > .oxd-input-group > :nth-child(2) > .oxd-input').type('Admin')
    cy.get(':nth-child(3) > .oxd-input-group > :nth-child(2) > .oxd-input').type('1')
    cy.intercept('POST','https://opensource-demo.orangehrmlive.com/web/index.php/auth/validate').as('validasi')
    cy.get('.oxd-button').click()
    ...
    it.only("LOG_010-Login failed when invalid username but password is valid", () => {
    cy.visit('https://opensource-demo.orangehrmlive.com/')
    cy.get(':nth-child(2) > .oxd-input-group > :nth-child(2) > .oxd-input').type('1')
    cy.get(':nth-child(3) > .oxd-input-group > :nth-child(2) > .oxd-input').type('Admin123')
    cy.intercept('GET','https://opensource-demo.orangehrmlive.com/web/index.php/auth/login').as('invlogin')
    cy.get('.oxd-button').click()
    cy.wait('@invlogin').its('response.statusCode').should('eq',200,201,304)
    cy.get('body').should('not.contain.text', 'Invalid credentials')
